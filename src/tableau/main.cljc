(ns tableau.main
  "Kotodama WASM entrypoint for the Tableau clean-room actor (L4) — Clojure port.
  ...
  facts (`tableau.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "tableau")
(def tier "L4")
...
(def entity-specs
  [{:entity "Dataset"    :plural "datasets"    :id-prefix "tableau_dat"
    :fields [:name :source :schemaRef :rowCount] :required [:name :source]
    :coerce {:rowCount :int}    :refs {}}
   {:entity "Query"      :plural "queries"     :id-prefix "tableau_que"
    :fields [:datasetId :sql :ownerId]         :required [:sql]
    :coerce {}                  :refs {:datasetId "Dataset"}}
   {:entity "Dashboard"  :plural "dashboards"  :id-prefix "tableau_das"
    :fields [:name :ownerId :layout]           :required [:name :layout]
    :coerce {}                  :refs {}}
   {:entity "Report"     :plural "reports"     :id-prefix "tableau_rep"
    :fields [:dashboardId :name :schedule]     :required [:name :schedule]
    :coerce {}                  :refs {:dashboardId "Dashboard"}}
   {:entity "Connection" :plural "connections" :id-prefix "tableau_con"
    :fields [:name :type :host :active]        :required [:name :type]
    :coerce {:active :bool}     :refs {}}
   {:entity "Metric"     :plural "metrics"     :id-prefix "tableau_met"
    :fields [:datasetId :name :expression :unit] :required [:name :expression]
    :coerce {}                  :refs {:datasetId "Dataset"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"tableau.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tableau-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tableau-compat/ready)
